...
((xmemb (car L1) L2) (eliminate (cdr L1) L2)) ;front of L1 is in L2,remove
front
...
((null? (cdr L)) L) ;L empty return L
 ((< (car L) (cadr L)) (cons (car L) (order (cdr L)))) ;front is small,
order is alright
 (#t (cons (cadr L) (order (cons (car L) (cddr L))))) ;front is not
small, swap
 )
)

...
function calc_linLSQ_line(data) {
  ...
  ***********************/
    ...
    let p = numeric.dot(numeric.dot(numeric.inv(numeric.dot(numeric.transpose(A), A)), numeric.transpose(A)), b);
  ...
  for(let i=0;i<N;++i) {
    ...
    ***********************/
      ...
      let p_temp = y[i] - model_out;
      p_temp = p_temp * p_temp;
      sse += p_temp;
  }
...
function calc_linLSQ_poly(data,order) {
  ...
  */
    ...
    let sse = 0;
    for (let i = 0; i < N; ++i) {
        let model_out = eval_poly_func(x[i], p);
        let p_temp = y[i] - model_out;
        p_temp = p_temp * p_temp;
        sse += p_temp;
    }
